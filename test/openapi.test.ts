...
const toJsonSchema = (s: StandardSchemaV1): unknown => {
  const { "~standard": _omit, ...json } = s as unknown as Record<string, unknown>;
  return json;
};
...
class DocUsersController {
  @get("/", {
    query: schema({ type: "object", properties: { q: { type: "string" } }, required: ["q"] }),
  ...
  @post("/", {
    body: schema({ type: "object", properties: { name: { type: "string" } }, required: ["name"] }),
  })
  create() {
    return {};
  }
...
describe("openapi() document shape", () => {
  ...
  test("custom info and servers are applied", () => {
    ...
    expect(d.info).toEqual({ title: "My API", version: "2.1.0", description: "hi" });
    expect(d.servers).toEqual([{ url: "https://api.example.com" }]);
  });

  test("routes become paths with :param converted to {param}", () => {
    expect(Object.keys(doc.paths).sort()).toEqual(["/health", "/users", "/users/{id}"]);
  });
