...
describe("Config", () => {
  test("get() coerces to the fallback's type", () => {
    const cfg = containerWith({ PORT: "8080", DEBUG: "true", NAME: "svc" }).resolve(Config);
  ...
  test("number coercion falls back on NaN", () => {
    expect(containerWith({ PORT: "nope" }).resolve(Config).get("PORT", 3000)).toBe(3000);
  });
  ...
  test("falls back to Bun.env when no source is registered", () => {
    (Bun.env as Record<string, string | undefined>).TURNOVER_TEST_KEY = "from-env";
    try {
      expect(new Container().resolve(Config).get("TURNOVER_TEST_KEY")).toBe("from-env");
    } finally {
...
describe("value() helper & createApp({ config })", () => {
  test("value() reads config in a field initializer (coerced)", async () => {
    ...
    const app = await createApp({ controllers: [CfgController], config: { PORT: "9090" } });
    expect(await (await app.handle(new Request("http://t/cfg"))).json()).toEqual({
      port: 9090,
    });
  });
  ...
  test("config accepts a ConfigSource object", async () => {
    ...
    const app = await createApp({ controllers: [SrcController], config: source });
    expect(await (await app.handle(new Request("http://t/src"))).json()).toEqual({
      v: "src",
    });
  });
});
...
describe("@profile gating", () => {
  test("a controller mounts only when its profile is active", async () => {
    ...
    const dev = await createApp({ controllers: [DebugController], profiles: ["dev"] });
    expect((await dev.handle(new Request("http://t/debug"))).status).toBe(200);
    const prod = await createApp({ controllers: [DebugController], profiles: ["prod"] });
  ...
  test("no @profile → always mounted", async () => {
    ...
    const app = await createApp({ controllers: [AlwaysController], profiles: ["prod"] });
  ...
  test("@profile matches any of several names", async () => {
    ...
    const app = await createApp({ controllers: [MultiController], profiles: ["test"] });
  ...
  test("Config.hasProfile reflects the active profiles", async () => {
    ...
    class PController {
      ...
      @get("/")
      show() {
        return { dev: this.cfg.hasProfile("dev"), prod: this.cfg.hasProfile("prod") };
      }
    }
    const app = await createApp({ controllers: [PController], profiles: ["dev"] });
  ...
  test("@profile on a module gates the whole module", async () => {
    ...
    expect((await prod.handle(new Request("http://t/mod/inner"))).status).toBe(404);
    const dev = await createApp({ modules: [DevModule], profiles: ["dev"] });
    expect((await dev.handle(new Request("http://t/mod/inner"))).status).toBe(200);
  });
});
