...
describe("body parsers", () => {
  ...
  test("a registered parser handles its content type", async () => {
    const app = await createApp({ controllers: [ParseController], parsers: [csv] });
  ...
  test("a subtype wildcard matches", async () => {
    const app = await createApp({ controllers: [ParseController], parsers: [shout] });
    const res = await app.handle(
      new Request("http://t/p", {
        method: "POST",
...
describe("response serializers", () => {
  const envelope: ResponseSerializer = {
    serialize(value) {
      if (value !== null && typeof value === "object" && !(value instanceof ReadableStream)) {
  ...
  test("wraps a value (envelope)", async () => {
    const app = await createApp({ controllers: [SerController], serializers: [envelope] });
    expect(await (await app.handle(new Request("http://t/s/obj"))).json()).toEqual({
      data: { id: 1 },
    });
  });
  ...
  test("content negotiation via the Accept header", async () => {
    const app = await createApp({ controllers: [SerController], serializers: [xml] });
  ...
  test("a streaming serializer handles a ReadableStream return", async () => {
    const app = await createApp({ controllers: [SerController], serializers: [streamer] });
