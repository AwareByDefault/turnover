...
const app = await createApp({ controllers: [UsersController, HealthController] });
...
describe("typed client", () => {
  ...
  test("GET with query params", async () => {
    const { data } = await api.get("/users", { params: { query: { limit: 5 } } });
    expect(data).toEqual([{ id: "1", name: "Ada" }]);
  });

  test("POST with a typed body returns 201", async () => {
    const { data, response } = await api.post("/users", { body: { name: "Bob" } });
