...
function resolveOrigin(
  ...
  if (Array.isArray(option)) {
    return option.includes(requestOrigin) ? { value: requestOrigin, vary: true } : null;
  }
...
export function cors(options: CorsOptions = {}): Plugin {
  ...
  const onRequest: RequestHook = (req) => {
    ...
    const requestHeaders = allowed ?? req.headers.get("access-control-request-headers");
    if (requestHeaders) headers.set("access-control-allow-headers", requestHeaders);
