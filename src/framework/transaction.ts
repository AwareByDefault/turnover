...
import { ctxMeta, type Ctor, metadataOf, TRANSACTIONAL } from "./metadata";
...
export function transactionalProcessor(container: Container): PostProcessor {
  return (instance, token: Ctor) => {
    const methods = metadataOf(token)?.[TRANSACTIONAL] as Set<PropertyKey> | undefined;
    ...
    return new Proxy(instance, {
      get(target, prop) {
        ...
        if (typeof prop === "string" && methods.has(prop)) {
          return (...args: unknown[]) => {
            const manager = container.resolveOptional(TRANSACTION_MANAGER, NOOP_MANAGER);
