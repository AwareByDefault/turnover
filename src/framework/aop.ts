...
import { ADVICE, ctxMeta, type Ctor, metadataOf } from "./metadata";
...
function adviceFor(context: ClassMethodDecoratorContext): AdviceSet {
  ...
  let set = map.get(context.name);
  if (!set) {
    set = { before: [], after: [], around: [] };
    map.set(context.name, set);
  }
  meta[ADVICE] = map;
  return set;
}
 ...
 * Self-invocation inside a method reaches the raw object (advice bypassed), as
 * in Spring AOP.
 */
