 ...
 */
// biome-ignore lint/suspicious/noExplicitAny: macros are string-keyed; args are dynamic
...
export function macro(name: string, ...args: unknown[]) {
  return (
    ...
    if (context.kind === "class") {
      ...
      const map =
        (meta[METHOD_MACROS] as Map<PropertyKey, MacroApplication[]> | undefined) ??
 ...
 */
export function expandMacros(applications: readonly MacroApplication[]): Required<MacroHooks> {
  const merged: Required<MacroHooks> = { use: [], derive: [], intercept: [], catchError: [] };
